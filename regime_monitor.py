...
V18 修復（2026-04-27）-- 來自 V17.0 策略複審報告 🟡-5：
  • 統一制度標籤：「震盪」→「震盪市」，與 tab_index.py / tab_multi_scan.py 一致
  • 影響：REGIME_RECOMMENDATIONS 跨模組查詢不再失效
"""
...
REGIME_STYLES = {
    ...
    # 🟡-5 V18：「震盪」→「震盪市」（與 tab_index / tab_multi_scan 一致）
    "震盪市":   {"color": "#64748b", "emoji": "⚪", "tier": "neutral"},
...
def _classify_regime(gap_pct: float, macd_pct: float, cov_20: float):
    ...
    if abs(gap_pct) < 2:
        # 🟡-5 V18：「震盪」→「震盪市」
        return "震盪市" if cov_20 > 2 else "轉折期"
