...
# A. 全預設策略模式 — 跑所有 ACTIVE 預設，輸出完整矩陣
...
# 🔴-3：自定義模式 buy_custom 擴到 11、sell_custom 擴到 8

# 🟡-1：滑桿預設 0.20% → 0.10%（與 walkforward / backtest Tab 一致）

# • 矩陣模式只跑 ACTIVE_PRESETS（不跑 LEGACY 對照組，避免無意義數字）
...
# V18：矩陣只跑 ACTIVE 策略；自定義 / 個別查找仍用 STRATEGY_PRESETS（含 LEGACY）

from config import (
ACTIVE_PRESETS, STRATEGY_PRESETS,
PRESET_NAMES, PRESET_CUSTOM,
)
...
with st.expander("⚙️ 參數設定", expanded=True):
    ...
    with c2:
        ...
        # 🟡-1 V18：滑桿 0.20 → 0.10
        rm_slip    = st.slider(
            "純滑點 % (不含手續費)", 0.0, 1.0, 0.10, 0.05, key="rm_slip",
            help="0.13% 手續費由 backtest 自動疊加。預設 0.10% + 0.13% = 單邊 0.23% / 雙邊 0.46%",
        ) / 100
...
# ══════════════════════════════════════════════════════════════
if rm_mode == "📊 全預設策略矩陣":
    # V18：只跑 ACTIVE 策略池，LEGACY 對照組已從矩陣排除
    n_strats = len(ACTIVE_PRESETS)
    st.info(
        f"預計跑 **{n_strats} 個 ACTIVE 策略** × 約 **{est_folds} 個 Fold** × {len(stocks)} 隻股票，"
        f"全程約 **{n_strats * 2}–{n_strats * 3} 分鐘**。"
        f"（LEGACY 對照組策略已從矩陣排除）"
    )

    run_clicked = st.button(
        f"🚀 開始跑全部 {n_strats} 個 ACTIVE 策略",
    ...
    if run_clicked:
        ...
        for si, (strat_name, strat_cfg) in enumerate(ACTIVE_PRESETS.items()):
            ...
            bucket = _run_one_strategy(
                ...
                min_hold_days=strat_cfg.get("min_hold_days"),
                cooldown_days=strat_cfg.get("cooldown_days"),
            )
    ...
    st.divider()
    st.markdown("### 📊 制度 × 策略矩陣")
    st.caption("格式：均每筆 OOS 回報%（筆數 勝率%）")
...
# ══════════════════════════════════════════════════════════════
    ...
    if _custom:
        ...
        # 🔴-3 V18：補 b11
        b11 = c2.checkbox("⑪ KDJ超賣金叉",     key="rm_b11")
        ...
        # 🔴-3 V18：補 s8
        s8  = d2.checkbox("⑲ KDJ高位死叉",     key="rm_s8")
    ...
    # 抓 preset 的 hold/cooldown 設定（自定義時為 None）
    if _custom:
        preset_min_hold = None
        preset_cooldown = None
    else:
        cfg = STRATEGY_PRESETS.get(_preset, {})
        preset_min_hold = cfg.get("min_hold_days")
        preset_cooldown = cfg.get("cooldown_days")
