...
# ══════════════════════════════════════════════════════════════════
#
# V18 修復（2026-04-27）-- 來自 V17.0 策略複審報告 🟡-8：
#   • 「N 天前」改用交易日（bar 索引）而非日曆日。
#     原版週五觸發、週一掃描會顯示「3 天前」，實際只過 1 個交易日。
#   • _strategy_triggered_recently 與 _sell_signal_hits 統一改用 trading-day diff。
# ══════════════════════════════════════════════════════════════════
...
from config import (
    ACTIVE_PRESETS, STRATEGY_PRESETS, B_NAMES, S_NAMES,
...
REGIME_RECOMMENDATIONS = {
    ...
    "弱牛市": [
        "💎+s2 M30 三重出場版【實盤冠軍】",            # WF +7.67% 主力
        "💎+ M30 RSI進雙出MIN30",                  # 進取版備用
        "💎M30 純粹均值回歸MIN30",                   # 經典基準
        "⚡ 突破確認",                              # 趨勢加成
    ],
    "牛市警惕": [
        "💎+s2 M30 三重出場版【實盤冠軍】",
    ...
    "熊市觀察": [
        "💎M30 純粹均值回歸MIN30",                   # 保守基準
        "🔄🔄M30 均值回歸長持MIN30",                # 雙條件更安全
    ],
    "弱熊市":   [],   # 空 = 停止掃描
    "強熊市":   [],
    "震盪市": [
...
# ══════════════════════════════════════════════════════════════════
# 🟡-8 V18：交易日 diff helper
# ══════════════════════════════════════════════════════════════════
def _trading_days_ago(series: pd.Series) -> int:
    """
    從一個 boolean Series 算出「最後一個 True」距離最後一個 bar 的「交易日數」。
    回傳 0 = 今天觸發，1 = 1 個交易日前，依此類推；無 True 回 -1。

    舊版用 (last_index - true_index).days 是日曆日，週五→週一會多算 2 天。
    新版用「位置差」（bar 索引差）才是交易日數。
    """
    if not series.any():
        return -1
    # last_true_pos = series 中最後一個 True 的位置
    # last_pos = series 中最後一個 bar 的位置
    last_pos = len(series) - 1
    # numpy where 拿到 True 的位置陣列
    true_positions = series.values.nonzero()[0]
    if len(true_positions) == 0:
        return -1
    last_true_pos = true_positions[-1]
    return int(last_pos - last_true_pos)


# ══════════════════════════════════════════════════════════════════
# 掃描一隻股票某個策略在過去 N 個交易日內是否觸發
# ══════════════════════════════════════════════════════════════════
def _strategy_triggered_recently(
    pre_sigs: dict, buy_tuple: tuple, lookback_days: int,
) -> int:
    """
    回傳：若過去 lookback_days 個交易日內有任一日完全觸發此策略，回傳「幾個交易日前觸發」（0=今天）；
          未觸發回 -1。
    ...
    # 🟡-8 V18：取最近 lookback_days+1 根 bar，用交易日位置差計算
    recent = combined.tail(lookback_days + 1)
    return _trading_days_ago(recent)


def _sell_signal_hits(pre_sigs: dict, lookback_days: int) -> list:
    """
    掃描單隻股票在過去 lookback_days 個交易日內觸發的賣出訊號列表。
    回傳 [(signal_name, days_ago), ...]，days_ago 為交易日數。
    """
    hits = []
    for k, name in enumerate(S_NAMES):
        # 🟡-8 V18：交易日邏輯
        series = pre_sigs[name].tail(lookback_days + 1)
        days_ago = _trading_days_ago(series)
        if days_ago >= 0:
...
def _days_label(days_ago: int) -> str:
    """🟡-8 V18：標籤改為「交易日」，避免和日曆日混淆"""
    if days_ago <= 0: return "今天"
    if days_ago == 1: return "1 交易日前"
    return f"{days_ago} 交易日前"
...
def _render_buy_mode(stocks: list, regime_info: dict, lookback: int,
    ...
    if use_all_strategies:
        # V18：「全策略」也只用 ACTIVE，避免 LEGACY 對照組混進掃描結果
        target_strategies = list(ACTIVE_PRESETS.keys())
        mode_label = f"全部 {len(target_strategies)} 個 ACTIVE 策略"
    else:
        target_strategies = REGIME_RECOMMENDATIONS.get(regime, [])
        mode_label = f"當前制度推薦 {len(target_strategies)}/{len(ACTIVE_PRESETS)} 個策略"
...
def render(stocks: list):
    ...
    with col1:
        lookback = st.selectbox(
            ...
            help="過去 N 個交易日內有任一日觸發策略即算命中（已修正為交易日，不是日曆日）",
        )
    ...
    if mode.startswith("🟢"):
        ...
        with col2:
            use_all = st.checkbox(
                ...
                help="預設只掃描當前制度推薦的策略以強調紀律。勾選後掃描全部策略。",
            )
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        with col2:
            st.caption(
                "💡 掃描你手動輸入的持倉清單，檢查過去 N 個交易日內是否觸發任何賣出訊號。"
                "2+ 訊號 = 強烈建議出場，1 訊號 = 警惕。"
            )
