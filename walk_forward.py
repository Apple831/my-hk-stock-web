...
while start + is_days + oos_days <= total_days:
    ...
    # ── BUG FIX 3 ──────────────────────────────────────────────
    # 原代碼問題：
    #   1. sell_map 的 key 是從字符串字段 "賣出日期" 清理後得來的，
    #      但持倉中交易的日期字符串格式不穩定，導致 key 匹配失敗，
    #      部分 OOS 交易的盈虧完全沒有計入 equity curve。
    #   2. 應直接使用 "_sell_date"（原生 Timestamp 對象）做日期索引，
    #      避免任何字符串格式問題。
    #   3. OOS equity curve 應用複利計算，與 backtest.py 保持一致。
    #
    # 修復方案：
    #   - 用 "_sell_date" Timestamp 建立 sell_map，與 oos_df.index 直接比對
    #   - 用 running_capital 複利追蹤，不再做百分比算術加總
    #   - 持倉中交易（"賣出日期" 含 "持倉中"）也正確納入最後一日的 equity

    if oos_trades:
        # 用原生 Timestamp 作 key，完全避免字符串格式問題
        sell_map: dict[pd.Timestamp, list] = {}
        for t in oos_trades:
            sell_date = t["_sell_date"]
            sell_map.setdefault(sell_date, []).append(t["回報%"])
