...
st.set_page_config(page_title="港股狙擊手 V3 - 宏觀版", layout="wide")
...
with tab1:
    ...
    with st.spinner("正在獲取大盤數據..."):
        hsi = yf.download("^HSI", period="6mo", interval="1d", progress=False)
...
# ================= 頁籤 3：個股分析 =================
with tab2 if 'tab3' not in locals() else tab3: # 相容舊版標籤名
    search = st.text_input("輸入代碼 (例如 9988.HK)", "0700.HK").upper()
    if st.button("查看數據", key="search_btn"):
        df_s = yf.download(search, period="6mo", interval="1d", progress=False)
        if not df_s.empty:
